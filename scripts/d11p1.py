...
class Monkey:
    ...
    def throw(self):
        ...
        target_monkey = self.throw_to[self.test(revised_worry)]
        print(
            f"Item with worry level {revised_worry} is thrown to monkey {target_monkey.id}"
        )
        target_monkey.grab(revised_worry)
        self.inspected += 1
        return
    ...
    def add_op(self, x, op_str, y):
        op = operators[op_str]
        if x == y:
            self.op = lambda a: op(a, a)
        else:
            self.op = lambda a: op(a, int(y))

    def add_test(self, test_divide_num):
        self.test = lambda a: a % test_divide_num == 0
