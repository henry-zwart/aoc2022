...
def main():
    ...
    available_space = TOTAL_DISKSPACE - root.size
    required_freeup = max(REQUIRED_SPACE - available_space, 0)
    ...
    while not queue.empty():
        dir = queue.get()
        if dir.size >= required_freeup and dir.size < min_dir.size:
            min_dir = dir
        for subdir in dir.subdirectories.values():
            queue.put(subdir)

    print(required_freeup)
    print(min_dir.size)
